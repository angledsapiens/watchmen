 ...
 */
export function initializeBelief(
  claimId: string,
  prior: number = 0.5,
...
export function updateBelief(
  ...
  // --- state transition ---
  let state: Belief["state"] = "tentative";
  if (confidence >= 0.75) state = "strong";
  else if (confidence <= 0.25) state = "weak";
  if (confidence === 0) state = "revoked";
